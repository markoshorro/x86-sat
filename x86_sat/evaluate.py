...
from .util import Value, try_simplify, try_simplify, is_z3, extend, match_types
...
def try_eval(ctx, e):
    return e.eval(ctx) if isinstance(e, Node) or "Var" in str(e.__class__) else e
...
def equal(a, b):
    ...
    if not isinstance(a, Node):
        return a == b
    return all(equal(getattr(a, param), getattr(b, param)) for param in type(a).params)
...
def serialize(value, t):
    ...
    assert m == 0
    return [
        z3.simplify(z3.Extract((i + 1) * width - 1, i * width, value)) for i in range(n)
    ]
...
class BinaryOp:
    def eval(self, ctx):
        [lhs, rhs, width, signed] = match_types(
            try_eval(ctx, self.lhs), try_eval(ctx, self.rhs)
        )
...
SCALE_SIZE = {"bit": 1, "byte": 8, "word": 16, "dword": 32, "qword": 64}
...
class Slice:
    ...
    #     ctx = {"expr": self.expr, "hi": self.hi, "lo": self.lo, "width": self.width}
    #     return ctx

    # def __setstate__(self, ctx):
    #     for k in ctx:
    #         setattr(self, k, ctx[k])
    ...
    def eval(self, ctx):
        ...
        if isinstance(self.target, Slice):
            ...
            assert isinstance(name, str), (
                "slice assignment to non-identifier: %s" % name
            )
...
class If:
    ...
    def __repr__(self):
        else_block = (
            ...
            if isinstance(self.else_block, Block) and self.else_block.stmts
            else ""
        )
        return "IF %s\n%s\n%sFI" % (self.expr, indent(self.if_block), else_block)
...
class Call:
    def eval(self, ctx):
        ...
        if isinstance(fn, Function) or hasattr(fn, "run"):
            fn = fn.run
        ...
        for i, (frame, *_) in enumerate(traceback):
            ...
            if isinstance(self, Node) and not isinstance(self, Block):
                ...
                if self.info:
                    print(
                        'File "%s", line %s, in %s'
                        % (self.info.filename, self.info.lineno, node_type)
                    )
                else:
...
def unwrap(values):
    # Ugly hack: for some unknown reason, isinstance returns False
    return [
        v.value if isinstance(v, Value) or ("Value" in str(v.__class__)) else v
        for v in values
    ]
...
def check(*assertions, for_all=[], return_type="str"):
    ...
    SOLVER.reset()
    CONDITION = z3.BitVec("aux", 128) == 0x0
    ...
    if return_type == "str":
        model = {
            v.name(): "%#0*x" % ((model[v].size() + 3) // 4 + 2, model[v].as_long())
            for v in model
        }
        ...
        assert (
            False
        ), 'return type must be one of "str", "int", "var", or "serial:<type>"'
    return (result, model)
