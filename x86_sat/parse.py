...
tokens = [
    ...
    ["INTEGER", (r"(0x[0-9A-Fa-f]+)|[0-9]+", lambda t: t.copy(value=int(t.value, 0)))],
    ["LPAREN", r"\("],
    ["RPAREN", r"\)"],
    ["LBRACKET", r"\["],
    ["RBRACKET", r"\]"],
...
rules = [
    ...
    [
        "trailing",
        ("PERIOD IDENTIFIER", lambda p: (Attr, p[1])),
    ...
    [
        "comp",
        (
            "xor_expr ((EQUALS|NOT_EQUALS|LESS_THAN|LESS_EQUALS|GREATER_THAN|"
            "GREATER_EQUALS|OP) xor_expr)*",
            reduce_binop,
        ),
    ],
    ["ternary", ("comp QUESTION comp COLON comp", lambda p: If(p[0], p[2], p[4]))],
    ["expr", "ternary|comp"],
    ...
    [
        "assignment",
        (
            "identifier (PLUS_EQUALS|MINUS_EQUALS|LSHIFT_EQUALS|RSHIFT_EQUALS) expr",
            lambda p: Assign(p[0], BinaryOp(p[1].replace("=", ""), p[0], p[2])),
        ),
    ],
    [
        "if_trail",
        ("ELSE if_stmt", lambda p: p[1]),
    ...
    [
        "if_stmt",
        (
            "IF (expr NEWLINE|parenthesized) stmt_list if_trail",
    ...
    [
        "case_stmt",
        (
            "CASE parenthesized OF NEWLINE (integer COLON stmt)+ ESAC",
    ...
    [
        "for_stmt",
        (
            "FOR identifier ASSIGN expr (TO|DOWNTO) expr NEWLINE stmt_list ENDFOR",
            lambda p: For(
                p[1], p[3], p[5], p[7], step=(1 if p[4].upper() == "TO" else -1)
            ),
        ),
    ],
    ...
    [
        "def_stmt",
        (
            "DEFINE IDENTIFIER LPAREN params RPAREN LBRACE stmt_list RBRACE",
    ...
    [
        "stmt",
        (
            "[assignment|if_stmt|case_stmt|for_stmt|while_stmt|return_stmt|def_stmt] "
            "(NEWLINE|SEMICOLON)",
            lambda p: p[0],
        ),
    ],
...
class ParseOperation:
    def __init__(self, name, params, return_var, return_type, operation):
        operation = operation.replace("\t", " " * 4)
        lex_ctx = lexer.input(operation, filename=name)
        try:
            self.tree = parser.parse(lex_ctx)
        ...
        self.name = name
        self.params = [
            Var(name, type, signed=get_type_signed(etype))
            for [name, type, etype] in params
        ]
        self.return_var = return_var
        self.return_type = return_type
        self.operation = operation
        self.width = get_type_width(self.return_type)
    ...
    def run(self, *args, **ctx_args):
        ...
        assert len(args) == len(self.params), (
            "wrong number of arguments, got %s, " "expected %s"
        ) % (len(args), len(self.params))
        for p, a in zip(self.params, args):
            a = Value(a, signed=p.signed, width=p.width)
            ctx.set(p.name, a)
        dst = Var(self.return_var, self.return_type).eval(ctx)
        ctx.set(self.return_var, dst)
        ...
        try:
            ...
            return e.value
        self.ctx = ctx
        return ctx.get(self.return_var)

    def __call__(self, *args, **ctx_args):
        f = Function(
            self.name,
            self.params,
            None,
            ...
            run=self.run,
            code=self.operation,
            width=get_type_width(self.return_type),
        )
        return f(*args, **ctx_args)
...
#     params = [
#         Var(name, type, signed=get_type_signed(etype)) for [name, type, etype] in params
#     ]

#     # Create a wrapper function for this intrinsic
#     def run(*args, **ctx_args):
#         print("RUN INSIDE CLOSURE")
#         ctx = Context(**ctx_args)
#         # Set up arguments
...
#         run=run,
#         code=operation,
#         width=get_type_width(return_type),
#     )
...
class IntrinsicMetadata:
    ...
    def __getattr__(self, name):
        if name not in self.fn_table:
            ...
            self.fn_table[name] = ParseOperation(name, *args)
    ...
    for intrinsic in root.findall("intrinsic"):
        ...
        if version < (3, 5, 0):
            ...
            [return_var, return_type] = [
                (r.attrib.get("varname", "dst"), r.attrib["type"])
                for r in intrinsic.findall("return")
            ][0]
